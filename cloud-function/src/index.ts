...
const upstreamUploadUrl =
  process.env.UPSTREAM_UPLOAD_URL || DEFAULT_UPSTREAM_UPLOAD_URL;
const publicAppBaseUrl =
  process.env.PUBLIC_APP_BASE_URL || DEFAULT_PUBLIC_APP_BASE_URL;
...
export const uploadProxy: HttpFunction = async (req, res) => {
  ...
  const upstreamResponse = await Sentry.continueTrace(
    { sentryTrace, baggage },
    async () => {
        ...
        async () => {
          if (traceId) {
            ...
            Sentry.captureMessage(
              `Cloud Function accepted upload [${traceId}]`,
              "debug",
            );
          }
  ...
  if (!upstreamResponse.ok) {
    if (traceId) {
      Sentry.captureMessage(
        `Cloud Function upstream upload failed [${traceId}]`,
        "error",
      );
    }
