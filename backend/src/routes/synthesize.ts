...
synthesizeRouter.post('/synthesize-with-captions', async (ctx) => {
  ...
  try {
    ...
    const [captions, translatedText] = await Promise.all([whisperPromise, translatePromise])

    // Whisper returned empty array → treat as null
    const finalCaptions = Array.isArray(captions) && captions.length > 0 ? captions : null
