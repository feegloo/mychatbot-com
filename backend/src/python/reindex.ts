 ...
 */
export async function ensureCollectionIndexed(
  ...
  const countResp = await fetch(
    `${config.pythonServerUrl}/collection-count/${encodeURIComponent(collectionName)}`,
  )
  ...
  if (!files.length && config.storageProvider === 'gcs' && config.gcsBucket) {
    ...
    if (storageKeys.length) {
      console.log(
        `[reindex] Downloading ${storageKeys.length} file(s) from GCS for ${collectionName}...`,
      )
  ...
  if (!files.length) {
    console.warn(
      `[reindex] Collection ${collectionName} is empty and no files available for re-indexing`,
    )
    return false
  }

  console.log(
    `[reindex] Collection ${collectionName} is empty — re-indexing ${files.length} file(s)...`,
  )
