...
ANSWER_PROMPT = ChatPromptTemplate.from_messages([
    ...
    h) Action buttons: When you want to suggest a follow-up action to the user (like creating a diagram, quiz, summary, checklist, etc.), do NOT write it as inline text. Instead, output action markers using this format: [action:Short label]. For example, instead of writing "mogę też stworzyć diagram albo quiz", write: [action:Create diagram] [action:Create quiz]. The label should be short (2-5 words) and clearly describe the action. You can place multiple action markers next to each other. Always place them at the very end of your answer, after all content.
