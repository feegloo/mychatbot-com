...
- IMPORTANT: The 7 buttons MUST follow this layout pattern:
  ...
  * **EMOJI VERIFICATION (mandatory before output)**: Before writing the final action line, scan every position 3–7 label. If any label is missing a trailing emoji, append an appropriate one. This applies especially to photo/image analysis actions — labels like "Describe in one sentence", "Write accessible alt text", "List visible clothing", "Create a stock photo caption", "Draft a social media caption", "Summarize the setting and mood", "Identify likely photo use", "Translate the description" MUST all end with emoji: 💬, ♿, 👗, 📝, 📱, 🌅, 📸, 🌐 respectively.
